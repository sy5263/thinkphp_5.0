...
class Article extends Controller
{
   ...
   $this->db =new \app\common\model\Article();
	...
	
    {
        ...
        //资讯管理查询搜索
        if(request()->isPost()){
            $key = input('param.article');
           // halt( $key);
            //halt($key);
            $field = db('article')->where('article_id|article_title','like',"{$key}%")->select();
        }
       //halt($field);
        if($field){
          ...
          //  halt($file);
            ...
            if($info){
              ...
              //  echo $info->getExtension();
               ...
               $datas['article_image']='/thinkphp_5.0/public/'.'upload'.'/'.$info->getSaveName();
            }else{
                // 上传失败获取错误信息
                echo $file->getError();
            }
           ...
           if($res['vaid'])
            {
                $this->success($res['aa'],'Article/article');exit;
            }else{
    ...
    if($res){
        $this->success('删除成功','article');exit;
    }else{
        ...
        if (request()->isPost())
        {
         ...
         // halt($file);
          if(!is_null($file)){
            ...
            if($res['vaid'])
            {
                $this->success($res['message'],'Article/article');exit;
